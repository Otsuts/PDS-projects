...
def get_data(train_dataset, test_dataset):
    train_loader = DataLoader(dataset=train_dataset, batch_size=train_dataset.__len__(), shuffle=True)
...
def write_log(w, args):
    file_name = args.log_path + '/' + datetime.date.today().strftime('%m%d') + \
                f"_{args.method}_{args.k_neighbors}_{args.metric_learning}.log"
